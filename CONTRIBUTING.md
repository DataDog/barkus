...
Welcome! We are glad you are interested in contributing to Barkus. This guide will help you understand the requirements and guidelines to improve your contributor experience.
...
This repository requires you to sign the [Contributor License Agreement](https://github.com/DataDog/datadog-cla/blob/main/CLA.md).

### Setting up your development environment

Barkus is a Rust workspace with Go (CGo) bindings. You will need:

- **Rust** — stable toolchain (edition 2021). Install via [rustup](https://rustup.rs/).
- **Go 1.21+** — required for the Go bindings and Go CLI. Install from [go.dev](https://go.dev/dl/).
- **Make** — used to orchestrate cross-language builds.
- **A C linker** — required by CGo. On macOS this is provided by Xcode Command Line Tools (`xcode-select --install`); on Linux, `gcc` or `clang`.

```bash
# Clone the repository
git clone https://github.com/DataDog/barkus.git
cd barkus

# Build the Rust CLI
cargo build -p barkus-cli --release

# Build the FFI library (required before any Go work)
make ffi
...
```bash
# Run the full test suite (Rust + Go)
make test

# Rust tests only
cargo test --workspace

# Go tests only (builds FFI automatically)
make test-go
```
...
Please make sure both `cargo clippy` and `go vet` pass with no warnings before opening a PR.
...
This repository includes an [AGENTS.md](AGENTS.md) that your AI code assistant should reference if you are using these tools as part of your contributions (if available).

## Contributing to issues

### Contributing to reporting bugs

If you think you have found a bug in Barkus feel free to report it.
